...
// Random function for grain/rain effect
float random(vec2 st) {
    return fract(sin(dot(st.xy, vec2(12.9898, 78.233))) * 43758.5453123);
}

// Cosine gradient color palette
...
void main() {
    ...
    vec2 grainUv = vUv * 100.0;
    float grain = random(grainUv + uTime);
    grain = smoothstep(0.5, 0.52, grain); // Make grain more sparse
    
    // Combine grain with main color
    colour += grain * uGrainIntensity;
